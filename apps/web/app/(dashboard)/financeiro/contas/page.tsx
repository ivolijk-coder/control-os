      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <section className="h-fit rounded-2xl border border-border-subtle bg-surface-1 p-5">
          ...
          <form className="space-y-4" onSubmit={(event) => void submit(event)}>
            ...
            {message && <p className="rounded-lg bg-tint/[0.04] px-3 py-2 text-sm text-text-secondary">{message}</p>}
            <div className="flex gap-2"><button disabled={saving} type="submit" className="rounded-lg bg-brand px-4 py-2.5 text-sm font-medium text-brand-ink hover:bg-brand-hover disabled:opacity-50">{saving ? 'Salvando…' : editing ? 'Salvar alterações' : 'Criar conta'}</button>{editing && <button type="button" onClick={resetForm} className="rounded-lg px-3 py-2.5 text-sm text-text-secondary hover:bg-tint/[0.05]">Cancelar</button>}</div>
          </form>
        </section>
      </div>
    </main>
