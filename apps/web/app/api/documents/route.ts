...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, code: error instanceof DocumentError ? error.code : 'UNKNOWN', message }, { status: error instanceof DocumentError && error.code === 'FILE_TOO_LARGE' ? 413 : 400 });
  }
}
