...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
 ...
 const document = await setDocumentArchived(userId, params.id, true); return document ? NextResponse.json({ success: true, document }) : NextResponse.json({ error: 'Documento não encontrado.' }, { status: 404 });
}
